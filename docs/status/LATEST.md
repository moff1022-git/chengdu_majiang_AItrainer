...
> 2026-07-29 — **F0028-3 Done；Human 换三张实体牌混用已修复，等待人工复测确认**
...
| 人工测试 | MT-01/02/05 Pass；MT-03 牌局正常；MT-04 修复后 GUI 已运行，操作结论待用户确认 |
...
- 修复自动验收为定向 23 passed、全量 322 passed / 1 skipped；GUI 复测日志未复现异常，窗口操作结果待用户确认。
- 人工验收总判定：**待 MT-04 用户确认**；详见 `F0028_3_MANUAL_QUICK_ACCEPTANCE_2026-07-29.md`。
...
| 序 | 动作 | 产出 / 依赖 | 建议触发语 |
|----|------|-------------|------------|
| 1 | 确认 MT-04 窗口操作结果（立即下一步） | Human 完成换三张、定缺、至少 10 次出牌；依赖用户反馈 | `MT-04 已进入定缺并完成 10 次出牌` |
| 2 | 补验 MT-03 策略列表并执行完整场景 | 显示项确认、MT-06～17、2/3 人、性能体感；依赖快速集放行 | `继续执行 F0028-3 完整人工测试` |
| 3 | 编写、确认并实现 F0028-4 | 认知、人格、注意力、记忆、满意停止和有界噪声；依赖人工阻塞清零 | 按 F0028-4 触发 |
| 4 | 编写、确认并实现 F0028-5–6 | 审计回放 → 训练契约 | 按对应 F 编号触发 |
