...
> 2026-07-29 — **F0030 逐玩家认知与目标参数 Approved，进入实现**
...
|----|------|
| 应用版本 | 0.2.1（本轮未变） |
| 规格主线 | F0028、F0029 `Done`；F0030 `Approved`，实现中 |
| 版本线 | state schema 5（reader 1–5）/ persistence 1 / wire 1 / PlayerView 2 |
| 测试门禁 | 353 passed / 1 skipped in 31.02s；GUI 156 控件/34 下拉框通过 |
...
| 序 | 动作 | 产出 / 依赖 | 建议触发语 |
|----|------|-------------|------------|
| 1 | 确认并实现 F0030（立即下一步） | 配置 1.1 迁移、逐座运行时读取、设置窗口与审计 | `确认并实现 F0030` |
| 2 | 执行 F0029/F0030 GUI 人工验收 | 四座独立认知参数、保存及下局生效 | 实现后执行 |
| 3 | 补验 F0028 完整人工场景 | MT-06～17、2/3 人、认知/审计/训练体感 | `继续执行 F0028 完整人工测试` |
| 4 | 验证真实历史存档 | schema 1–4 真实夹具和迁移报告 | `导入并验证真实历史存档夹具` |
| 5 | 规划发版或下一功能 | 依赖人工验收 | `准备发布` 或 `规划下一功能` |
