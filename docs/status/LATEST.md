...
- 2026-08-02 F0036：能力测试工具规格扩展为支持 humanlike_v2 人格预设记录和 1/5/10/20/50/100 并发线程选项；针对性测试 `10 passed`。
...
- 2026-08-02 F0036：batch 模式改为每个座位选择 AI 后立即选择该座位 humanlike_v2 人格预设，再进入下一座位。针对性测试 `14 passed`。
- 2026-08-02 F0036：修复 batch 模式并发参数未生效问题；batch 现在使用线程池执行待测 game_id，结果按 game_id 排序并记录线程数，断点恢复沿用配置线程数。针对性测试 `14 passed`。
