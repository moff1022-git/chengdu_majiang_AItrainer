...
> 2026-07-29 — **F0028-3 自动验收 Done；快速人工验收发现 Human 换三张阻塞缺陷**
...
| 人工测试 | MT-01/02/05 Pass；MT-03 牌局正常；MT-04 **Fail/Blocker** |
| Git | 人工方案基线 `c80ee0a`；本轮失败记录待提交；远端未推送 |
...
- MT-04 阻塞失败：Human face Tile 与 AI PhysicalTile 在 opening 换牌结算中混入同一手牌，排序触发类型比较异常。
- 人工验收总判定：**不通过**；详见 `F0028_3_MANUAL_QUICK_ACCEPTANCE_2026-07-29.md`。
...
| 序 | 动作 | 产出 / 依赖 | 建议触发语 |
|----|------|-------------|------------|
| 1 | 修复 Human 换三张实体牌解析（立即下一步） | opening face→PhysicalTile resolver、混合路径测试、全量回归；依赖根因已定位 | `修复 Human 换三张实体牌混用问题` |
| 2 | 复测 MT-04 | Human 完成换三张、定缺、至少 10 次出牌；依赖修复通过 | `复测 F0028-3 MT-04` |
| 3 | 补验 MT-03 策略列表并执行完整场景 | 显示项确认、MT-06～17、2/3 人、性能体感；依赖快速集放行 | `继续执行 F0028-3 完整人工测试` |
| 4 | 编写、确认并实现 F0028-4 | 认知、人格、注意力、记忆、满意停止和有界噪声；依赖人工阻塞清零 | 按 F0028-4 触发 |
