...
> 2026-07-29 — **F0028 人类化 AI v2（F0028-1–6）全部 Done**
...
|----|------|
| 应用版本 | 0.2.1（本轮未变） |
| 规格主线 | F0028 `Done`；F0028-1–6 全部 `Done` |
...
| Git | F0028-6 Approved 文档基线已提交；实现与验收待本轮收尾提交；远端未推送 |
...
| 序 | 动作 | 产出 / 依赖 | 建议触发语 |
|----|------|-------------|------------|
| 1 | 补验 F0028 完整人工场景（立即下一步） | 策略显示、MT-06～17、2/3 人、认知/审计/训练体感 | `继续执行 F0028 完整人工测试` |
| 2 | 验证真实历史存档 | schema 1–4 真实夹具和迁移报告；发布前建议完成 | `导入并验证真实历史存档夹具` |
| 3 | 规划下一功能或发版 | 依赖 F0028 Done 与人工补验结果 | `规划下一功能` 或 `准备发布` |
| 4 | 推送本地 main | 外部状态变更，仍需明确目标远端/分支 | `将本地 main 推送到 origin` |
| 5 | 整理 OneDrive 冲突副本 | 先只读列清单；删除另行授权 | `整理 OneDrive 冲突副本，先出清单` |
