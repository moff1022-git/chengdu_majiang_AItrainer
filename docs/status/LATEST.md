...
| Git | F0028-6 文档与实现已提交；F0029 Draft 待本轮文档提交；远端未推送 |
...
| 序 | 动作 | 产出 / 依赖 | 建议触发语 |
|----|------|-------------|------------|
| 1 | 确认并实现 F0029（立即下一步） | 参数服务/窗口、大厅/主桌/AI 座位开关；依赖规格 Approved | `确认并实现 F0029` |
| 2 | 执行 F0029 GUI 人工验收 | 大厅 800×600、主桌、AI 窗口、参数保存/下局生效 | 实现完成后自动进入 |
| 3 | 补验 F0028 完整人工场景 | MT-06～17、2/3 人、认知/审计/训练体感 | `继续执行 F0028 完整人工测试` |
| 4 | 验证真实历史存档 | schema 1–4 真实夹具和迁移报告 | `导入并验证真实历史存档夹具` |
| 5 | 规划发版或下一功能 | 依赖 F0029/F0028 人工验收 | `准备发布` 或 `规划下一功能` |
| 6 | 推送本地 main | 外部状态变更，确认目标远端/分支 | `将本地 main 推送到 origin` |
