...
实现与验收见 [`F0029_ACCEPTANCE_2026-07-29.md`](../status/F0029_ACCEPTANCE_2026-07-29.md)。

## 11. 中文表单纠偏（2026-07-29）

用户明确要求 Humanlike Settings 不得采用 JSON/文档修改模式。参数窗口必须改为中文人机交互表单：每个设置项显示中文名称、用途说明、当前值和合法范围；布尔值用开关、枚举用下拉、数值/文本用受校验输入，复杂数组使用专用文本区但不得暴露整份配置 JSON。该要求是对本规格 §4.3 的实现纠偏，直接进入修复，不另设功能范围。

后续可用性反馈进一步明确：按钮必须保证系统主题下高对比可读；所有可调数值必须展示 validator 的实际上下限或关联约束；所有自由文本必须展示长度/非空规则；所有有限集合文本必须使用只读下拉框，不得让用户手输内部枚举值。
