...
| **状态** | **`Draft`**（待确认后实现脚本；须在 **Windows 主机** 上构建验收） |
...
| **关联代码（计划）** | `app_paths.py`（已支持 win32 可写路径）、`packaging/windows/*`、`tools/packaging/build_*_windows.*`、既有 `main --seat-window` |
...
**开放**：是否默认只维护 PyInstaller、Nuitka 作可选（实现时两者都交付脚本，Release 可只挂 PyInstaller）。
