...
| **后续增强** | 玩家窗几何 → **[F0001](../features/F0001_window_geometry.md)**；**多 human（≤3）→ [F0020](../features/F0020_multi_human_modes.md)** |
| **规格修订** | 下文「最多 1 human」为 **M09 首版**；**程序当前以 F0020 为准（≤3）** |
...
| 项 | 默认决议 |
|----|----------|
| 双窗口 | **已演进**：含 human 时主进程 **开** 主 GUI 观战 + 座位窗（F0002/F0003） |
| human 数量 | 首版最多 1；**现 F0020：最多 3**（布局 A/B/D） |
| 超时 | 默认 **120s**（可配置） |
| 分析 | 主进程计算 hints 下发 |

**开放问题 — 已关闭（用户确认 M09，2026-07-10；后经 F 系列演进）：**

| # | 决议 | 现状 |
|---|------|------|
| 1 | 含 human 时主进程不开观战 GUI | **已改**：主程序 + 座位窗并存 |
| 2 | 首版最多 1 个 human | **已改**：见 **F0020**（≤3） |
| 3 | 决策超时默认 120s | 仍有效 |
