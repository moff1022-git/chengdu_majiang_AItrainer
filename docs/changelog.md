...
## 2026-08-06 — F0070主代理自动编排契约

- 新增F0070主代理总控规格，定义按DAG自动生成规格/实现/验证/审计子代理、路径互斥和独立审计要求。
- 固化CLI重启后的幂等续跑协议：核对工作区/HEAD/证据SHA，忽略过期心跳，以恢复队列续派。
- 明确自动授权不绕过Docs-First、平台沙箱、破坏性/外部动作、最终盲测、新语义和第三次重复finding人工门禁。
- 本轮仅修改文档；未修改`task19_agent_runtime`、monitor、业务代码或实验数据。
