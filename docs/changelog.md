...
- Humanlike并发审计：12局workers 1/2/5耗时均约42秒，峰值RSS由56.64升至102.91 MiB；记录线程无吞吐收益和外部终止风险，未改变runner架构。

## 2026-08-05 — F0057/F0058审计与正式人格快照

- F0057：候选trace增加原始向听、定缺张数、有效进张牌面及公开剩余计数；仅增强审计，不改变策略。
- F0058：能力测试报告增加逐座解析后人格参数快照；设置窗口雷达固定四项决策权重轴顺序并增加无GUI验收。
- 正式`nonhuman_optimized`入口在固定100局中`100/100`成功，s0总分`97`、胡`34`，与F0055实验一致。
- Humanlike扩大回归`121 passed`；全仓发现一个已退役F0011合同的过期测试，留待先对齐规格后处理。

## 2026-08-03 — nonhuman 弃牌向听离线分析

- 使用项目 `engine.shanten()` 对新 nonhuman 1000 局每次 s0 discard 的手牌前后计算向听变化，并按花猪/非花猪分层。
- 花猪层平均向听 `4.1887→3.8931`；非花猪层 `3.7056→3.3064`。引擎多数中早盘状态不提供 ukeire，报告明确标记进张数据限制，未用空集合冒充有效 0 进张。

## 2026-08-03 — nonhuman 弃牌 trace 分层统计

- 对修复后 nonhuman 1000 局的 s0 discard 决策按最终花猪/非花猪分层，统计定缺剩余牌数、候选最高/次高分及分差、候选数、碰后首次出牌。
- 结果保存至 `data/ai_capability/results/1000_trace_nonhuman_v2/.../discard_analysis.json`；当前 trace 没有标准向听/进张字段，后续需离线手牌分析补齐。

## 2026-08-03 — nonhuman 修复后固定牌局复跑

- 使用同一 `fairness-20260802-fair-004` 1000 局固定数据集重新运行 nonhuman 对照组；新建 `1000_trace_nonhuman_v2`，未删除旧结果。
- 结果 `1000/1000` 成功，1000/1000 steps trace 与 audit trace；新 trace 验证 `preset_id=nonhuman_optimized`、有效 search depth `8`、满意阈值 `1.0`。

## 2026-08-03 — nonhuman 1000 局逐局分析

- 对 `fairness-20260802-fair-004` 同一 1000 个 game_id 的 nonhuman 与 expert 完整 trace 做配对分析：nonhuman s0 总分 `-277` 对 expert `282`，花猪 `53` 对 `40`，胡牌 `317` 对 `358`。
- 初步定位为 nonhuman 的进攻偏好与局部 evaluator 不匹配：碰/杠增加但胡牌减少、花猪增加，且 s0 P95 响应约为 expert 的 1.86 倍；未发现发牌公平性证据。
- 进一步按 `state_hash_before` 逐手对齐：37197 个共同状态、622 局首处分叉，全部发生在弃牌阶段；记录 hand-value/defense 权重和 plan 分叉，以及 configured search depth 8 → effective 4、threshold 1.0 → 0.95 的实际生效偏差。
- 修复 nonhuman 预设运行时参数合同：`search_depth=8` 不再被 expert 上限截断为 4，`satisfaction_threshold=1.0` 不再被统一上限截断为 0.95；trace 记录 `preset_id`，并新增 2 项回归断言。
- 记录后续固定牌局 A/B、定缺/进张特征分层和搜索 profiling 任务。
...
- Release v0.3.1：从远程 `main` 的 `a0d5031f` 干净克隆分别以 PyInstaller 6.21.0、Nuitka 4.1.3 构建 macOS arm64 App；版本、座位窗口、资源和 ZIP 完整性检查通过，两个安装包及 SHA-256 清单已上传至 GitHub Release。
- Packaging：发布构建补收 `players/humanlike/parameter_registry_v2.json`；仓库构建脚本的永久修复仍列为后续任务。
## 2026-08-03

- 完成 nonhuman 固定牌局 1000 局参数 A/B，新增完整复盘结果与分析文档。
- F0040：胡牌候选仅受 GP-009 过胡许可控制，不再受 satisfaction_threshold 延迟或放弃。
- F0041：trace 增加计划状态、计划重启和 GP-009 胡牌候选审计字段。
- 完成 F0041 两组 1000 局固定牌局审计重跑，旧数据保留。
- F0042：一次性采集完整人格参数与必要审计字段，删除无效采集并重跑最终两组 1000 局。
- 完成两组完整 audit 离线统计，确认计划重启过密导致 plan_persistence 实际失效，HU mandatory 一致性通过。
- 完成 53 个花猪局逐手因果分析，区分 52 个机会约束局与 1 个 evaluator 策略错误局。
- 完成 nonhuman/expert 1000 局胡牌结果与结算原因损失分解，定位主要损失为少自摸/少胡与花猪尾部。
- 完成 104 个 expert-only 胡局首分叉离线权重反事实，定位 hand_value/speed 组合为主要参数原因。
- F0043：完成固定 1000 局权重 A/B，确认权重修正恢复 41 个胡牌并提升 462 分。
- 完成权重修正后剩余 97 分分解，定位自摸价值差和单局花猪结算边界为主要剩余项。
- 阶段性超越分析：确认剩余差异集中在杠后路径和终局结算边界，尚未修改正式配置。
- 完成非杠方向离线检查，确认计划重启过敏和近等候选路径是下一优先方向，未修改正式配置。
- F0044：增加最后一张定缺牌禁杠门禁并通过 11 项测试；1000 局回归未超过 expert。
- 完成任务 1–4 离线分析，量化计划重启和 big_hand_preference 对候选路径的影响。
- F0045：完成 big_hand_preference 0.80/0.60/0.40 离线反事实验证，下一步仅对候选值做固定牌局 A/B。
## 2026-08-04
- F0054–F0056：建立三数据集联合门禁并筛选30,400个弃牌决策；F0055合并9000局+977、95% CI下界>0，正式Nonhuman晋级gang `.50`及权重`.40/.20/.25/.15`，定向28 passed。
- F0053：完成F0052的28局逐手分叉分析并验证仅暗杠Expectimax；主1000局+11，但seed-a -19、seed-b -2，合并9000局-10且CI跨0；业务实现回滚，正式预设不变。
- F0052：完成公开信息两层杠后Expectimax与17项定向测试；100局96分通过门禁，但固定1000局234分、较旧`.50`/Expert低48分，完整trace 1000/1000；业务实现回滚，正式预设不变。
- F0051：完成27面公开概率补牌枚举与100局门禁；82分低于旧模型91分，回滚业务实现，未运行1000局。下一步转两层expectimax。
- F0050：完成杠后EV规格、实现及小样本门禁；100局从旧模型91分降至-45分，门禁失败，业务实现回滚，未运行1000局，正式预设不变。
- F0049：恢复独立公平生成器；seed-a/seed-b 各4000局、四组A/B零失败，总10000局配对统计完成。`.55`合计+44但95% CI跨0且seed-a为负，正式预设不变。
- F0050：新增杠后期望收益模型 Draft，作为固定 `gang_preference` 未通过万局门禁后的下一方向。
- F0048：完成 gang `.35–.60` 离线邻域筛选、`.55` 主数据集完整1000局及held-out `.50/.55` 各1000局验证；`.55` 合计领先17分但95% CI跨0，未修改正式预设。
- 状态基线：更新 `DOC_CODE_BASELINE.md` 至应用0.3.1、本地 `c2e6d571` 与远程main `a0d5031f` 的当前事实。
- F0047：完成 `gang_preference=.50` 固定 1000 局 A/B；s0 从 171 提升到 282，与 Expert 全部 1000 局终局逐局一致，完整 trace 与参数快照验证通过。
- 修正 F0043 过期的 185 分记录；磁盘 `games.jsonl` 权威值为 171。
- 完成 Nonhuman `big_hand_preference=0.60` 固定牌局 A/B，并记录完整复盘统计。
