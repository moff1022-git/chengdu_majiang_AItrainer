...
# PyInstaller spec for Windows — Chengdu Mahjong AI Trainer
# Prefer: tools/packaging/build_pyinstaller_windows.ps1
...
if _spec.name == "windows" or (_spec / "pyinstaller_entry.py").is_file():
    SPEC_DIR = _spec if _spec.is_dir() else _spec.parent
else:
    SPEC_DIR = _spec.parent
ROOT = SPEC_DIR.parents[1] if SPEC_DIR.name == "windows" else SPEC_DIR.parent
ENTRY = SPEC_DIR / "pyinstaller_entry.py"
if not ENTRY.is_file():
    ENTRY = ROOT / "packaging" / "windows" / "pyinstaller_entry.py"
...
datas = [
    ...
    (
        str(ROOT / "players" / "humanlike" / "parameter_registry_v2.json"),
        "players/humanlike",
    ),
]
...
a = Analysis(  # type: ignore[name-defined]
    ...
    hooksconfig={},
    runtime_hooks=[],
    excludes=["pygame.tests", "pygame.examples", "cv2"],
